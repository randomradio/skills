...
SCRIPT_NAME="run_long_horizon_loop.sh"
...
usage() {
  cat <<'USAGE'
Long Horizon Objective-First Loop

Run an objective-first long-horizon loop with fresh-context work/review phases, file-backed state,
optional verification commands, and resumable progress.

Usage:
  run_long_horizon_loop.sh --cwd <dir> --prompt "text" [options]
  run_long_horizon_loop.sh --cwd <dir> --prompt-file <file> [options]
  run_long_horizon_loop.sh --cwd <dir> --objective-file <file> [options]
  run_long_horizon_loop.sh --cwd <dir> --resume [options]

Core options:
  --cwd <dir>                      Repository/workspace to run Codex in (required)
  --engine <codex|claude>          Execution backend (default: codex)
  ...
  --state-dir <dir>                State directory (default: <cwd>/.codex/long-horizon-loop)
...
Engine execution options:
  --codex-bin <path-or-name>       Engine CLI binary path (default: codex for codex engine, claude for claude engine)
  --engine-bin <path-or-name>      Alias for --codex-bin
...
Examples:
  run_long_horizon_loop.sh \
    --cwd /repo \
    --engine codex \
    --prompt "Implement auth flow" \
    --acceptance-file /repo/.codex/long-horizon-loop/acceptance-criteria.md \
    --validate-cmd "npm test" \
    --max-iterations 25

  run_long_horizon_loop.sh --cwd /repo --engine codex --resume
...
upsert_markdown_section() {
  ...
  python3 - "$file_path" "$heading" "$section_body" <<'PY'
import re
import sys

path, heading, body = sys.argv[1], sys.argv[2], sys.argv[3]
with open(path, "r", encoding="utf-8") as fh:
    text = fh.read()
...
seed_runtime_from_planner_prompt() {
  [[ -f "$planner_prompt_file" ]] || return 0

  local seeded=0
  local objective_seed=""
  local acceptance_seed=""

  if objective_seed="$(extract_markdown_section "$planner_prompt_file" "Objective" 2>/dev/null)"; then
    objective_seed="$(trim_ws "$objective_seed")"
  else
    objective_seed=""
  fi

  if acceptance_seed="$(extract_markdown_section "$planner_prompt_file" "Acceptance Criteria" 2>/dev/null)"; then
    acceptance_seed="$(trim_ws "$acceptance_seed")"
  else
    acceptance_seed=""
  fi

  if [[ -n "$objective_seed" && ! -s "$objective_state_file" ]]; then
    write_text_file "$objective_state_file" "$objective_seed"
    seeded=1
  fi

  if [[ -n "$acceptance_seed" && ! -s "$acceptance_state_file" ]]; then
    write_text_file "$acceptance_state_file" "$acceptance_seed"
    seeded=1
  fi

  if [[ "$seeded" -eq 1 ]]; then
    note "Seeded runtime objective/acceptance from $planner_prompt_file"
  fi
}

sync_planner_documents() {
...
- Iteration: $iteration
- Status: $status_label
- Work status: ${work_status_value:-"(not available)"}
- Review decision: ${review_decision_value:-"(not available)"}
- Validation: ${current_validation_status:-"(not run)"}
- Progress gate: ${current_progress_status:-"(not evaluated)"}
- Next step: ${work_next_step_value:-"(not available)"}
- Stop reason: ${stop_reason:-"(in progress)"}
EOF
)
  upsert_markdown_section "$planner_plans_file" "Loop Status" "$plans_body"

  docs_body=$(cat <<EOF
- Last updated: $(now_utc)
- Iteration: $iteration
- Reviewer outcome: ${review_decision_value:-"(not available)"}
- Reviewer assessment: ${review_assessment_value:-"(not available)"}
- Optional verification status: ${current_validation_status:-"(not run)"}
- Validation summary: ${current_validation_summary:-"(not run)"}
- Stop reason: ${stop_reason:-"(in progress)"}
EOF
)
  upsert_markdown_section "$planner_docs_file" "Runtime Execution Notes" "$docs_body"
}
...
adapter_file=""
LH_ENGINE_CMD=()
LH_ENGINE_EXPECTS_STDOUT_JSON=0
...
case "$engine" in
  codex|claude)
    ;;
  *)
    die "--engine must be one of: codex, claude"
    ;;
esac

if [[ -z "$state_dir" ]]; then
  state_dir="$cwd/.codex/long-horizon-loop"
fi
...
complete_marker_file="$state_dir/.loop-complete"
...
planner_prompt_file="$cwd/docs/long-horizon/Prompt.md"
planner_plans_file="$cwd/docs/long-horizon/Plans.md"
planner_docs_file="$cwd/docs/long-horizon/Documentation.md"
...
- Inspect .codex/long-horizon-loop/events.log and .codex/long-horizon-loop/codex/ for details.
...
- Inspect .codex/long-horizon-loop/events.log and .codex/long-horizon-loop/codex/ for review-phase details.
...
- Expected schema file: .codex/long-horizon-loop/work-schema.json
...
- Expected schema file: .codex/long-horizon-loop/review-schema.json
...
- Inspect .codex/long-horizon-loop/validation/iteration-$iteration/ logs.
...
build_work_phase_prompt() {
  ...
  cat <<EOF_PROMPT
Long Horizon Work Phase for Codex
Run ID: $run_id
Iteration: $iteration

You are the worker in a long-horizon objective-first loop. Start with fresh context. All continuity lives in repo files.
...
build_review_phase_prompt() {
  ...
  cat <<EOF_PROMPT
Long Horizon Review Phase for Codex
Run ID: $run_id
Iteration: $iteration

You are the reviewer in a long-horizon objective-first loop. Start with fresh context. Review the repo state and the work summary fairly and strictly.
...
if best is None:
    raise SystemExit("no JSON object found in engine output")

with open(output_path, "w", encoding="utf-8") as fh:
    fh.write(json.dumps(best, ensure_ascii=True))
    fh.write("\n")
PY
}
...
run_phase_exec_with_watchdog() {
  ...
  LH_ENGINE_CMD=()
  LH_ENGINE_EXPECTS_STDOUT_JSON=0
  lh_engine_build_command "$phase_name" "$local_prompt" "$output_file" "$schema_file" "$phase_model" "$phase_profile" "$phase_sandbox"
  local phase_cmd=("${LH_ENGINE_CMD[@]}")

  : > "$attempt_jsonl"
  : > "$attempt_stderr"
  log_event "engine_attempt_start" "engine=$engine;phase=$phase_name;attempt=$attempt;jsonl=$attempt_jsonl"

  (
    cd "$cwd"
    "${phase_cmd[@]}"
  ) >"$attempt_jsonl" 2>"$attempt_stderr" &
  local codex_pid=$!
  local start_ts
  start_ts="$(date +%s)"
  ...
  if [[ -n "$attempt_timeout_reason" ]]; then
    warn "engine exec timed out (engine=$engine, phase=$phase_name, reason=$attempt_timeout_reason, attempt=$attempt)"
    log_event "engine_timeout" "engine=$engine;phase=$phase_name;attempt=$attempt;reason=$attempt_timeout_reason;jsonl=$attempt_jsonl"
  ...
  cmd_status=$?
  set -e
  log_event "engine_attempt_end" "engine=$engine;phase=$phase_name;attempt=$attempt;exit=$cmd_status;jsonl=$attempt_jsonl"

  if [[ "$cmd_status" -eq 0 && "$LH_ENGINE_EXPECTS_STDOUT_JSON" -eq 1 ]]; then
    if ! extract_last_json_object "$attempt_jsonl" "$output_file"; then
      warn "engine output did not contain a valid JSON object (engine=$engine, phase=$phase_name)"
      return 1
    fi
  fi
  return "$cmd_status"
...
run_phase_with_retries() {
  ...
  while true; do
    ...
    if [[ "$phase_exit" -eq 124 && "$attempt" -lt "$max_attempts" ]]; then
      log_event "engine_timeout_retry" "engine=$engine;phase=$phase_name;attempt=$attempt;reason=${attempt_timeout_reason:-timeout}"
      attempt=$((attempt + 1))
      continue
    fi
    break
...
if [[ "$resume" -eq 1 ]]; then
  ...
  if [[ -z "$prompt" && -z "$prompt_file" && -z "$objective_file" ]]; then
    if prompt="$(extract_markdown_section "$planner_prompt_file" "Objective" 2>/dev/null)"; then
      prompt="$(trim_ws "$prompt")"
      if [[ -n "$prompt" ]]; then
        note "Using objective from $planner_prompt_file"
      fi
    fi
  fi
  [[ -n "$prompt" || -n "$prompt_file" || -n "$objective_file" ]] || die "Provide --prompt, --prompt-file, --objective-file, or define ## Objective in $planner_prompt_file"
...
if [[ "$engine" == "claude" && "$provided_codex_bin" -eq 0 && "$codex_bin" == "codex" ]]; then
  codex_bin="claude"
fi

adapter_file="$SCRIPT_DIR/lib/engines/${engine}.sh"
[[ -f "$adapter_file" ]] || die "Engine adapter not found: $adapter_file"
# shellcheck disable=SC1090
source "$adapter_file"
declare -F lh_engine_preflight >/dev/null 2>&1 || die "Adapter missing lh_engine_preflight: $adapter_file"
declare -F lh_engine_build_command >/dev/null 2>&1 || die "Adapter missing lh_engine_build_command: $adapter_file"
...
print_effective_config() {
  cat <<EOF_CONFIG
Long Horizon Loop Effective Configuration
...
note "Long-horizon loop started"
note "Run ID: $run_id"
note "State dir: $state_dir"
note "Stop file: $stop_file"
log_event "loop_start" "engine=$engine;autonomy=$autonomy_level;sandbox=$sandbox;codex_bin=$codex_bin;events_format=$events_format"
...
while true; do
  ...
  if run_phase_with_retries "work" "$work_prompt" "$work_last_message_file" "$work_schema_file" "$model" "$profile" "$sandbox"; then
    ...
    sync_planner_documents "in_progress"
    iteration=$((iteration + 1))
    save_state
    pause_between_iterations
    continue
  ...
  if [[ -n "$stop_reason" ]]; then
    sync_planner_documents "$stop_reason"
    break
  fi
...
active=0
save_state
sync_planner_documents "${stop_reason:-stopped}"
write_summary "$stop_reason"

note "Long-horizon loop stopped: $stop_reason"
